...
const getPersonaImage = (name: string): string => {
  ...
  if (name.includes('김민석') || name.includes('취준생') || name.includes('한 손'))
    return '/personaC.png';
  if (name.includes('Brian') || name.includes('미국인') || name.includes('여행객'))
    return '/personaD.png';
...
export default function Results() {
  ...
  useEffect(() => {
    if (enableBlindSpot) {
      const adhdIndex = results?.personas.findIndex(
        (p) => p.name.includes('이혁준') || p.name.includes('대리')
      );
  ...
  useEffect(() => {
    if (enableLocalBlocker) {
      const foreignerIndex = results?.personas.findIndex(
        (p) => p.name.includes('Brian') || p.name.includes('미국인')
      );
  ...
  useEffect(() => {
    if (enableThumbZone) {
      const oneHandIndex = results?.personas.findIndex(
        (p) => p.name.includes('김민석') || p.name.includes('취준생')
      );
  ...
  // 노안 시뮬레이터: 실제 노안의 특징을 반영
  // - 작은 글씨만 초점이 안 맞음 (전체적으로 흐리지 않음)
  // - 대비 감소 (특히 회색 텍스트)
  // - 수정체 황변으로 인한 색감 변화
  const getBlurSeverity = (
    coord: { x: number; y: number; width: number; height: number },
    feedback: string
  ) => {
    const area = coord.width * coord.height;
    let baseSeverity = 0.3;

    // 영역이 작을수록 더 흐리게 (작은 텍스트 = 읽기 어려움)
    if (area < 0.003) baseSeverity = 1.2; // 아주 작은 텍스트
    else if (area < 0.01) baseSeverity = 0.8; // 작은 텍스트
    else if (area < 0.03) baseSeverity = 0.5; // 중간 텍스트
    else baseSeverity = 0.3; // 큰 요소는 거의 영향 없음

    // 텍스트 관련 피드백이면 약간 증가
    if (feedback.includes('작') || feedback.includes('깨알')) baseSeverity += 0.3;
    if (feedback.includes('글씨') || feedback.includes('텍스트')) baseSeverity += 0.2;

    return Math.min(baseSeverity, 1.5);
  };

  // 노안의 대비 감소 효과 계산
  const getContrastReduction = (coord: { x: number; y: number; width: number; height: number }) => {
    const area = coord.width * coord.height;
    // 작은 요소일수록 대비가 더 떨어져 보임
    if (area < 0.005) return 0.75;
    if (area < 0.015) return 0.82;
    return 0.88;
  };
          ...
          <div>
            <div className='bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-4 lg:sticky lg:top-20 max-h-[calc(100vh-6rem)] overflow-auto'>
              <div className='flex justify-center pb-5'>
                {/* Ability Toggle - Unified White Style */}
                {activePersona.name.includes('할머니') && (
                  <Button
                    variant='outline'
                    size='sm'
                    className={cn(
                      'gap-2 transition-all duration-300 text-md rounded-xl',
                      enableElderlyVision
                        ? 'bg-red-500/20 hover:bg-red-500/30 text-white border-red-500/50 shadow-[0_0_20px_rgba(239,68,68,0.4)]'
                  ...
                  >
                    ...
                    <span
                      className={cn(
                        ...
                        enableElderlyVision ? 'bg-red-500 text-white' : 'bg-white/20 text-white'
                      )}
                    >
                      {enableElderlyVision ? 'ON' : 'OFF'}
                    </span>
                  </Button>
                )}
                {(activePersona.name.includes('이혁준') || activePersona.name.includes('대리')) && (
                  ...
                  >
                    ...
                    <span
                      className={cn(
                        'text-[10px] font-bold px-1.5 py-0.5 rounded ml-1',
                        enableBlindSpot ? 'bg-red-500 text-white' : 'bg-white/20 text-white'
                      )}
                    >
                      {enableBlindSpot ? 'ON' : 'OFF'}
                    </span>
                  </Button>
                )}
                {(activePersona.name.includes('Brian') ||
                  activePersona.name.includes('미국인')) && (
                  <Button
                    variant='outline'
                    size='sm'
                    className={cn(
                  ...
                  >
                    ...
                    <span
                      className={cn(
                        'text-[10px] font-bold px-1.5 py-0.5 rounded ml-1',
                        enableLocalBlocker ? 'bg-red-500 text-white' : 'bg-white/20 text-white'
                      )}
                    >
                      {enableLocalBlocker ? 'ON' : 'OFF'}
                    </span>
                  </Button>
                )}
                {(activePersona.name.includes('김민석') ||
                  activePersona.name.includes('취준생')) && (
                  <Button
                    variant='outline'
                    size='sm'
                    className={cn(
                  ...
                  >
                    ...
                    <span
                      className={cn(
                        'text-[10px] font-bold px-1.5 py-0.5 rounded ml-1',
                        enableThumbZone ? 'bg-red-500 text-white' : 'bg-white/20 text-white'
                      )}
                    >
                      {enableThumbZone ? 'ON' : 'OFF'}
                    </span>
                  </Button>
                )}
              </div>
              <div className='relative inline-block w-full'>
                  ...
                  <>
                    ...
                    <div className='absolute inset-0 pointer-events-none rounded-lg overflow-hidden'>
                      {activePersona.coordinates.map((coord, idx) => {
                        const blurValue = getBlurSeverity(coord, activePersona.feedback[idx]);
                        const contrastValue = getContrastReduction(coord);
                        return (
                          <div
                            key={`blur-${idx}`}
                            ...
                            style={{
                              ...
                              backdropFilter: `blur(${blurValue}px) contrast(${contrastValue}) saturate(0.9)`,
                              WebkitBackdropFilter: `blur(${blurValue}px) contrast(${contrastValue}) saturate(0.9)`,
                            }}
                          />
                        );
                      })}
                ...
                {enableBlindSpot &&
                  activePersona &&
                  (activePersona.name.includes('이혁준') ||
                    activePersona.name.includes('대리')) && (
                    <div className='absolute inset-0 pointer-events-none rounded-lg overflow-hidden'>
                      {activePersona.coordinates.map((coord, idx) => (
                        <div
                          key={`blackout-${idx}`}
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              width: `${coord.width * 100}%`,
                              height: `${coord.height * 100}%`,
                              backgroundColor: isUpperZone
                                ? 'rgba(239, 68, 68, 0.25)'
                                : isMiddleZone
                                ? 'rgba(234, 179, 8, 0.2)'
                                : 'rgba(34, 197, 94, 0.15)',
                              border: `2px solid ${
                                isUpperZone ? '#ef4444' : isMiddleZone ? '#eab308' : '#22c55e'
                              }`,
                              borderRadius: '4px',
                            }}
                          >
              ...
              {enableElderlyVision && activePersona?.name.includes('할머니') && (
                <div className='mt-4 p-3 bg-purple-500/10 backdrop-blur-xl border border-purple-500/30 rounded-lg'>
                  <p className='text-sm text-purple-200 flex items-start gap-2'>
                    ...
                    <span>
                      <strong className='text-purple-100'>노안 시뮬레이터 활성화:</strong> 75세
                      고령층의 시각을 시뮬레이션합니다. 작은 텍스트의 초점이 흐려지고, 대비가 낮은
                      요소가 잘 보이지 않으며, 수정체 황변으로 색감이 약간 탁해집니다.
                    </span>
                  </p>
                </div>
              )}
                ...
                (activePersona.name.includes('이혁준') || activePersona.name.includes('대리')) && (
                  <div className='mt-4 p-3 bg-gray-500/10 backdrop-blur-xl border border-gray-500/30 rounded-lg'>
                    <p className='text-sm text-gray-200 flex items-center gap-2'>
                      ...
                      <span>
                        <strong className='text-gray-100'>블라인드 스팟 활성화:</strong> 검은 영역은
                        ADHD 성향의 사용자가 읽지 않고 스킵한 텍스트입니다. 당신이 심혈을 기울인
                        카피가 실제로는 전달되지 않았을 수 있습니다.
                      </span>
                    </p>
                  </div>
                )}
                ...
                (activePersona.name.includes('Brian') || activePersona.name.includes('미국인')) && (
                  <div className='mt-4 p-3 bg-red-500/10 backdrop-blur-xl border border-red-500/30 rounded-lg'>
                    <p className='text-sm text-red-200 flex items-center gap-2'>
                      <span className='text-lg'>🌏</span>
                      <span>
                        <strong className='text-red-100'>로컬 블로커 활성화:</strong> 표시된 영역은
                        외국인 사용자가 이해하기 어렵거나 수행할 수 없는 절차입니다. 한국어 전용
                        콘텐츠, 현지 결제 시스템, 미번역 UI 등이 포함됩니다.
                      </span>
                    </p>
                  </div>
                )}
                ...
                (activePersona.name.includes('김민석') ||
                  ...
                  <div className='mt-4 p-3 bg-orange-500/10 backdrop-blur-xl border border-orange-500/30 rounded-lg'>
                    <p className='text-sm text-orange-200 flex items-center gap-2'>
                      <span className='text-lg'>👆</span>
                      <span>
                        <strong className='text-orange-100'>Thumb Zone 활성화:</strong> 한 손 조작
                        시 엄지가 닿기 어려운 영역을 표시합니다.
                        <span className='inline-flex items-center gap-1 ml-1'>
                          <span className='inline-block w-3 h-3 rounded bg-red-500'></span>닿기
                          어려움
                          <span className='inline-block w-3 h-3 rounded bg-yellow-500 ml-2'></span>
                          불편함
                          <span className='inline-block w-3 h-3 rounded bg-green-500 ml-2'></span>
                          편함
                        </span>
                      </span>
                    </p>
              ...
              <div className='flex items-center justify-between'>
                ...
                <div className='text-right'>
                  <Button
                    size='sm'
                    variant='secondary'
                    className='gap-2 border border-white/20 rounded-xl'
            ...
            <div className='grid grid-cols-2 gap-2'>
              {results.personas.map((persona, index) => (
                ...
                >
                  <div className='flex items-center gap-2'>
                    ...
                    <div className='truncate'>
                      <span className='text-xs text-gray-400'>
                        {getPersonaDescriptor(persona.name)}
                      </span>
                  ...
                  <span
                    className={cn('text-lg font-bold tabular-nums', getScoreColor(persona.score))}
                  >
                    {persona.score}
                  </span>
                </button>
            ...
            {activePersona && (
              <div className='bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-5'>
                  ...
                  <div className='flex items-center gap-3'>
                    ...
                    <div>
                      <p className='text-xs text-gray-400 mb-0.5'>
                        {getPersonaDescriptor(activePersona.name)}
                      </p>
                      <h3 className='font-bold text-lg text-white'>{activePersona.name}</h3>
                      <p className='text-xs text-gray-400'>
                        {activePersona.feedback.length}개의 문제 발견
                      </p>
                    </div>
                  </div>
                  <div className='flex items-center gap-3'>
                    ...
                    <div className='text-right bg-white/5 px-4 py-2 rounded-lg border border-white/10'>
                      ...
                      <p
                        className={cn(
                          ...
                          getScoreColor(activePersona.score)
                        )}
                      >
                        {activePersona.score}
                      </p>
                ...
                <div className='w-full bg-white/10 rounded-full h-2 mb-4 shadow-inner'>
                  <div
                    className={cn(
                      ...
                      getScoreBg(activePersona.score)
                    )}
                ...
                <div className='space-y-2'>
                  <h4 className='font-semibold text-xs text-gray-400 uppercase tracking-wide mb-2'>
                    발견된 문제
                  </h4>
      ...
      >
        ...
        >
          ...
          <div style={{ marginBottom: '32px' }}>
            <h2
              style={{ fontSize: '18px', fontWeight: '600', marginBottom: '16px', color: '#111' }}
            >
              분석된 화면
            </h2>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div>
                    ...
                    <h3 style={{ fontSize: '18px', fontWeight: 'bold', margin: 0, color: '#111' }}>
                      {persona.name}
                    </h3>
                    <p style={{ fontSize: '14px', color: '#666', margin: '4px 0 0 0' }}>
                      {persona.feedback.length}개의 문제 발견
                    </p>
                  </div>
                </div>
              ...
              >
                <div
                  style={{
                    ...
                    borderRadius: '4px',
                    backgroundColor:
                      persona.score >= 80 ? '#22c55e' : persona.score >= 60 ? '#eab308' : '#ef4444',
                  }}
                />
              </div>
              ...
              <div>
                ...
                {persona.feedback.map((feedback, feedbackIndex) => (
                  ...
                  >
                    ...
                    <p style={{ fontSize: '14px', margin: 0, color: '#111', lineHeight: '1.5' }}>
                      {feedback}
                    </p>
                  </div>
                ))}
