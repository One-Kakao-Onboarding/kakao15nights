...
**중요: 긍정적인 평가나 칭찬은 절대 하지 마세요. 오직 문제점과 개선이 필요한 부분만 지적해주세요.**
...
**엄지 도달 존 (모바일 기준):**
- y: 0.0 ~ 0.3 - 닿기 어려움 (Red Zone) - 문제 영역!
- y: 0.3 ~ 0.6 - 불편함 (Yellow Zone) - 주의 필요
...
- 주요 버튼(CTA)이 엄지손가락이 닿기 어려운 상단에 배치되어 있는가?
- 뒤로 가기, 닫기 등의 기능이 제스처(스와이프) 없이 상단 버튼으로만 되는가?
- 터치 타겟(버튼 크기)이 너무 작아서 엄지로 눌렀을 때 오작동이 발생하는가?
- 화면 가장자리에 중요한 버튼이 있어서 오작동(Ghost Touch)이 발생할 수 있는가?
- 텍스트 입력이 너무 많이 필요한가?`,
